...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        ...
        if (in_array($ext, $allowed)) {
            $new_name = uniqid() . '.' . $ext;
            $upload_path = '../images/actualites/' . $new_name;
            if (move_uploaded_file($_FILES['image']['tmp_name'], $upload_path)) {
                $image_url = 'images/actualites/' . $new_name;
            }
        }
    }

    $stmt = $pdo->prepare("INSERT INTO news (title, category, content, image_url) VALUES (?, ?, ?, ?)");
    if ($stmt->execute([$title, $category, $content, $image_url])) {
        $success = "L'actualité a été publiée avec succès.";
    } else {
...
<title>Publier une Actualité — HPCI CMS</title>
...
<style>
  ...
  .form-group input, .form-group select, .form-group textarea { width: 100%; padding: 14px; border: 1px solid #e2e8f0; border-radius: 8px; font-size: 1rem; color: #0a1628; }
  ...
  .error-msg { background: #fee2e2; color: #991b1b; padding: 16px; border-radius: 8px; margin-bottom: 24px; }
...
<body>
  ...
  <div class="main-content">
    <div class="admin-header">
      <a href="news.php" style="text-decoration: none; color: #64748b; font-weight: 700;">← Retour</a>
    ...
    <div class="form-card">
      <form action="" method="POST" enctype="multipart/form-data">
        <div class="form-group">
          ...
          <input type="text" id="title" name="title" required placeholder="Ex: Nouveau contrat de maintenance industrielle">
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          <label for="image">Image illustrative (Recommandé : 800x440px)</label>
          <input type="file" id="image" name="image" accept="image/*">
        </div>
        <div class="form-group">
          <label for="content">Contenu de l'article</label>
          <textarea id="content" name="content" required placeholder="Décrivez l'actualité ici..."></textarea>
        </div>
        <button type="submit" class="btn-submit">Publier l'actualité</button>
      </form>
    </div>
  </div>
</body>
